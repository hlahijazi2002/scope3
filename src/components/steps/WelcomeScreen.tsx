...
export default function WelcomeScreen() {
  ...
  return (
    <div className="min-h-screen flex font-['Poppins']">
      {/* ── LEFT — dark panel ── */}
      <div className="hidden lg:flex w-[42%] bg-[#0F5F4B] flex-col justify-between p-12 relative overflow-hidden">
        {/* texture circles */}
        <div className="absolute w-125 **:h-125 rounded-full border border-white/10 -top-40 -left-40 pointer-events-none" />
        <div className="absolute w-75 h-75 rounded-full border border-white/10 bottom-20 -right-20 pointer-events-none" />
        <div className="absolute w-50 h-50 rounded-full bg-[#1FA971]/20 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 blur-3xl pointer-events-none" />
        ...
        <div className="relative z-10">
          ...
          <p className="text-white/60 text-[14px] leading-relaxed mb-10 max-w-70">
        ...
        {/* bottom */}
        <div className="text-white/30 text-[11px] relative z-10">
          © {new Date().getFullYear()} URIMPACT Platform
        </div>
      </div>

      {/* ── RIGHT — content panel ── */}
      <div className="flex-1 bg-[#F9FAFB] flex items-center justify-center p-8 lg:p-14">
        <div className="w-full max-w-110">
          {/* mobile logo */}
          <div className="flex items-center gap-2 mb-8 lg:hidden">
            <div className="w-8 h-8 rounded-lg bg-[#0F5F4B] flex items-center justify-center">
              <Leaf size={15} className="text-white" />
            </div>
            <span className="font-bold text-[15px] text-[#1D1F21]">
              URIMPACT
            </span>
          </div>

          <h2 className="text-[28px] font-bold text-[#1D1F21] leading-tight mb-2">
            Ready to map your
            <br />
            emissions footprint?
          </h2>
          ...
          <div className="space-y-3">
            <button
              onClick={() => goToStep(1)}
              className="w-full flex items-center justify-between px-5 py-4 bg-[#0F5F4B] hover:bg-[#0a4a39] text-white rounded-xl font-semibold text-[14px] transition-all duration-200 hover:shadow-lg group"
            >
            ...
            <button
              onClick={() => goToStep(1)}
              className="w-full flex items-center justify-between px-5 py-4 bg-white hover:bg-[#F3FBF7] text-[#0F5F4B] border border-[#E5E7EB] hover:border-[#1FA971] rounded-xl font-medium text-[14px] transition-all duration-200 group"
            >
